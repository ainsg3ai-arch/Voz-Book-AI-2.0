...
const MiniPlayer: React.FC<MiniPlayerProps> = ({ currentBook }) => {
  ...
  return (
    <div 
      className="fixed bottom-[80px] md:bottom-6 left-4 right-4 z-30 md:left-[270px]"
      onClick={() => navigate(`/player/${currentBook.id}`)}
    >
      <div className="glass-panel flex h-[72px] items-center gap-4 rounded-2xl px-3 pr-5 shadow-glass cursor-pointer hover:bg-white/5 transition-all group overflow-hidden relative border border-white/10">
        {/* Progress Bar Background */}
        <div className="absolute bottom-0 left-0 h-[2px] w-full bg-white/5">
            <div className={`h-full bg-primary shadow-[0_0_10px_#3AB8FF] transition-all duration-1000 ${isPlaying ? 'w-full animate-[loading_30s_linear]' : 'w-[35%]'}`}></div>
        </div>

        {/* Rotating Art */}
        <div className="h-12 w-12 shrink-0 rounded-full overflow-hidden shadow-lg border border-white/10 group-hover:scale-105 transition-transform bg-[#1a232d]">
          {imgError ? (
            <div className="h-full w-full bg-gradient-to-br from-primary/20 to-purple-500/20 flex items-center justify-center">
             ...
             <img
                className={`h-full w-full object-cover opacity-90 ${isPlaying ? 'animate-[spin_10s_linear_infinite]' : ''}`}
        ...
        <div className="flex items-center gap-2 md:gap-4">
            ...
            >
                <span className="material-symbols-outlined text-2xl fill">{isPlaying ? 'pause' : 'play_arrow'}</span>
            </button>
             <button 
